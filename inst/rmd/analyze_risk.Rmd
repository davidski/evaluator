...
output:
  ...
  pdf_document:
    ...
    toc_depth: 3
  word_document:
    reference_docx: styles/word-styles-references.docx
    toc: yes
    toc_depth: 3
...
> This report is based on `r comma(attr(simulation_results, "simulation_count"))` 
simulations performed over `r comma(length(unique(simulation_results$scenario_id)))` 
...
max_loss %>% 
  group_by(outliers) %>% 
  purrrlyr::by_slice(~ arrange(.x, max_loss) %>% 
...
losses are `r domain_impact[[1, "domain"]]`, `r domain_impact[[2, "domain"]]`, and 
`r domain_impact[[3, "domain"]]`. The losses associated with each domain of the 
program are described in the following table.

```{r domain_impact, echo = FALSE}
mutate_at(domain_impact, vars(-one_of("domain_id", "domain")), funs(dollar)) %>% 
...
generate_event_outcomes_plot(control_weakness)
```
...
1. The number of times the threat community against the organization.
2. The capabilities of the threat community.
...
domain_loss_frequency <- simulation_results %>% group_by(domain_id, simulation) %>% 
  ...
  ungroup %>% 
  purrrlyr::slice_rows("domain_id") %>% 
  purrrlyr::by_slice(~ describe(.x$loss_events), .collate = "rows") %>% select(-vars)
...
plot_scenarios_by_domain <- function(x) {
  ...
  gg <- gg + labs(x = "Loss Events per Year", y = NULL,
                  ...
                  subtitle = "All Scenarios with Loss Events Included",
                  caption = "Source: Evaluator toolkit")
  gg <- gg + theme_evaluator(base_family = get_base_fontfamily())
  gg <- gg + theme(panel.grid.major = element_blank())
  gg <- gg + theme(panel.grid.minor = element_blank())
...
if (max(domain_summary$ale) >= risk_tolerance["high"] * .9) {
...
if (max(domain_summary$ale) >= risk_tolerance["medium"] * .9) {
...
gg <- gg + facet_wrap(~measure, nrow=2, "free_x", strip.position = "left", 
