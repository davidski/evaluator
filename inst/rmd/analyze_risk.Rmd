...
> This report is based on `r sum(map_dbl(simulation_results$results, ~attr(.x, "iterations")))`
...
create_table <- function(label, id) {
  ...
  filter(mappings, type == id) %>% 
    ...
    mutate(l = ifelse(id == "lm", dollar(l), 
                      ifelse(id == "tef", comma(l), percent(l)))) %>% 
    mutate(ml = ifelse(id == "lm", dollar(ml), 
                       ifelse(id == "tef", comma(ml), percent(ml)))) %>% 
    mutate(h = ifelse(id == "lm", dollar(h), 
                      ifelse(id == "tef", comma(h), percent(h)))) %>% 
