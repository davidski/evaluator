...
scenario_summary <- 
  scenario_summary %>% 
  # extract ale__var
  mutate(ale_var = map_dbl(.data$summary, "ale_var")) %>% 
...
> This report is based on `r sum(map_dbl(simulation_results$results, ~attr(.x, "simulation_count")))`
...
loss_exceedance <- summarize_simulations(simulation_results$results) %>% 
...
overall <- summarize_simulations(simulation_results$results)
tibble::tibble(
...
top_domains_by_var <- domain_summary %>% unnest() %>% 
...
```{r top_5_risks}
scenario_summary %>% unnest(summary, .drop = TRUE) %>% 
...
display_scenario_table <- function(scenario_summary, id) {
  scenario_summary %>% unnest(.data$summary) %>% 
...
```{r show_outliers}
filter(scenario_summary, outlier == TRUE) %>% unnest(summary, .drop = TRUE) %>% 
...
create_table <- function(label, id) {
  ...
  filter(mappings, type == id) %>% 
    select(label, l, ml, h, conf) %>% 
    rowwise() %>% 
    ...
    rename("Category" = label, "Low" = l, "Most Likely" = ml, "High" = h, 
           "Confidence" = conf) %>% 
    pander(justify = c("left", rep("right", 4)), caption = label)
}
```
...
analysis takes a simplified approach. An absolute upper bound of $16 
million for any single loss event is derived from the largest known HIPAA fine[^1].

```{r loss_magnitudes_table}
create_table("Loss Magnitudes", "lm")
```

[^1]: https://www.hhs.gov/hipaa/for-professionals/compliance-enforcement/agreements/anthem/index.html
...
unnest(scenario_summary, summary, .drop = TRUE) %>% 
  arrange(desc(ale_var)) %>% 
  filter(ale_var != 0) %>% 
...
gg <- ggplot(unnest(scenario_summary, summary, .drop = TRUE), 
...
simulations_per_domain <- simulation_results %>% group_by(domain_id) %>% 
  mutate(summary = map(results, summarize_simulations)) %>% 
...
loss_events_by_scenario <- scenario_summary %>% 
  mutate(n = map_dbl(summary, "loss_events_mean")) %>% 
...
outlier_domains <- scenario_summary %>% unnest(summary, .drop = TRUE) %>% 
...
dat <- unnest(simulation_results) %>% filter(domain_id %in% outlier_domains$domain_id) %>% 
  mutate(full_label = str_glue("{domain_id} - {scenario_id}")) %>% 
  arrange(full_label)
...
gg <- unnest(simulation_results) %>% filter(!scenario_id %in% scenario_outliers) %>% 
  mutate(scenario_id = factor(as.character(scenario_id), 
...
simulation_results %>% unnest() %>% group_by(scenario_id, simulation) %>% 
...
scenarios %>% left_join(domains, by = c("domain_id" = "domain_id")) %>%  
  mutate(n_controls  = map_dbl(scenario, ~length(.x$diff_params))) %>% 
  filter(n_controls == 1) %>% 
  arrange(domain_id, scenario_id) %>% 
...
```{r scenario_table}
arrange(quantitative_scenarios, domain_id, scenario_id) %>% 
...
left_join(quantitative_scenarios, unnest(scenario_summary, summary, .drop = TRUE), 
