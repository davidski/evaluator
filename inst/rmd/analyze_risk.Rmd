...
scenario_summary <- scenario_summary %>% unnest(.data$summary, .drop = TRUE) %>% 
  mutate(outlier = scenario_id %in% scenario_outliers)

domain_summary_new <- summarize_domains(simulation_results)
domain_summary <- tidyr::unnest(domain_summary)

max_loss <- calculate_max_losses(simulation_results, scenario_outliers)

simulation_results <- unnest(simulation_results)
...
scenario_summary %>% unnest(summary) %>% 
...
unnest(scenario_summary, summary) %>% 
  arrange(desc(ale_var)) %>% 
  filter(ale_var != 0) %>% 
...
domain_loss_frequency <- simulation_results %>% 
...
gg <- summarize_simulations(simulation_results$results) %>% 
  ggplot(., aes(x = loss_events))
...
left_join(quantitative_scenarios, scenario_summary, 
