...
simulation_results_new <- readRDS(file.path(results_directory, "simulation_results.Rds"))
simulation_results <- simulation_results_new

scenario_summary_new <- mutate(simulation_results_new, summary = map(results, summarize_scenario)) 
scenario_summary <- unnest(scenario_summary_new, summary, .drop = TRUE)

scenario_outliers <- identify_outliers(scenario_summary_new) %>% 
  filter(outlier == TRUE) %>% pull(scenario_id)

domains <- dat$domains
domain_summary_new <- summarize_domains(simulation_results_new)
domain_summary <- domain_summary_new %>% unnest() %>% 
  left_join(domains, by = "domain_id")
simulation_results <- unnest(simulation_results_new, results) %>% 
...
max_loss <- calculate_max_losses(simulation_results_new, scenario_outliers)
```
...
````{r var_valuebox}
overall <- simulation_results %>% group_by(iteration) %>% 
...
loss_events <- group_by(simulation_results, iteration) %>% 
...
gg <- generate_heatmap(domain_summary_new)
