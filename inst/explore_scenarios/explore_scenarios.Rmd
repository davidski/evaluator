...
get_summary_table <- function(sid) {
  ...
  summary_data <- mutate_at(summary_data, .funs = funs(dollar), 
     ...
     mutate(mean_tc_exceedance = ifelse(is.nan(mean_tc_exceedance), NA, custom_percent(mean_tc_exceedance))) %>% 
...
get_control_table <- reactive({
  ...
  # format percentages
  control_data <- mutate_at(control_data, vars(min, mode, max), list(custom_percent))
