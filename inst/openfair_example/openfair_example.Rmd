...
output: 
  flexdashboard::flex_dashboard:
    ...
    source_code: embed
    navbar:
      - { title: "About", href: "https://evaluator.severski.net", align: right }
---
...
observeEvent(input$runmodel, {
  ...
  LMestimate <- data.frame(L = input$lml, ML = input$lmml, H = input$lmh, 
	...
	TEFsamples <- rpert(values$iterations, TEFestimate$L, TEFestimate$ML, TEFestimate$H, 
	                    shape = TEFestimate$CONF)
	TCsamples <- rpert(values$iterations, TCestimate$L, TCestimate$ML, TCestimate$H, 
	                   shape = TCestimate$CONF)
	DIFFsamples <- rpert(values$iterations, DIFFestimate$L, DIFFestimate$ML, 
...
```{r}
numericInput("iterations", "# Iterations:", 10000, min = 1000, max = 100000, step = 1000)
