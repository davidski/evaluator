...
scenario_summary %>% arrange(desc(ale_var)) %>% 
  ...
  top_n(20, ale_var) %>% 
  ggplot(., aes(x = ale_var, y = reorder(scenario_label, ale_var), color=annual_tolerance)) -> gg
...
frequency. Scenarios with a z-score of 2 or higher experience more frequent 
loss events, on average.
...
gg <- results %>% filter(!scenario_id %in% scenario_outliers) %>% 
  ggplot(., aes(y = as.character(scenario_id), x = ale))
#gg <- gg + facet_grid(~domain_id, scales = "free", space="free", strip.position = "bottom")
...
gg <- gg + geom_point(position = "jitter", alpha = 1/5, 
                      aes(size = loss_events_median))
...
gg <- gg + labs(x = "Median ALE", y = "Maxmium ALE", 
                size = "Loss Event Count", 
