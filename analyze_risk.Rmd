...
focus_scenarios <- c(51, 11)    # if there are scenarios of particular interest, 
...
gg <- gg + geom_path()
gg <- gg + geom_vline(xintercept = 0.8, color = "red", size = 2) + 
...
dat <- dat %>% gather(type, events, c(loss_events, contained_events)) %>% 
  ...
  mutate(nudge = ifelse(type=="loss_events", -8000, 8000)) %>% 
...
gg <- ggplot(dat, aes(x = events, xend=0, 
                y=domain_id)) -> gg
gg <- gg + geom_segment(aes(yend = domain_id), color = "steelblue")
gg <- gg + geom_point(color = "steelblue", size = 2)
gg <- gg + geom_text(aes(label=full_lab, x = events + nudge, y=domain_id, hjust=hjust, 
                         family="Museo Sans Cond 300"), size = 3)
gg <- gg + geom_label(aes(x=0, y=domain_id, label=domain_id),  size=3, label.size=NA)
gg <- gg + scale_x_continuous(expand=c(0,0), breaks=c(-200000, 0, 200000), 
...
which make up `r nrow(domains)` security program areas. The 
...
When aggregated to the domain level, risk is taken as the maximum loss scenario 
within that domain.
...
gg <- gg + guides(size = guide_legend(title="Median Annual Loss", title.position = "top"))
gg <- gg + guides(color = guide_legend(title="Total Losses vs Risk Tolerance", title.position = "top"))
gg <- gg + theme_evaluator()
gg <- gg + theme(panel.grid.minor = element_blank())
gg
```
...
gg <- gg + scale_x_continuous(labels = comma)
gg <- gg + scale_y_continuous(labels = dollar_millions)
#gg <- gg + scale_color_brewer(palette = "Blues", type="seq", drop= FALSE)
gg <- gg + scale_color_continuous(labels = dollar_millions)
...
gg <- gg + labs(x = "Scenario ID", y = "Annual Losse Exposure",
...
### Median vs Maximum Annual Losse Exposure
...
This is a limitation of the interactions between scenarios and domains not being defined.
