...
realize into losses over the course of a single year resulting in higher 
totally losses.
...
```{r caclulate_weak_domains}
control_weakness <- scenario_summary %>% group_by(domain_id) %>% 
  summarize(vuln = percent(round(mean(mean_vuln), 2))) %>% 
  arrange(desc(vuln)) %>% 
  ungroup()
```
...
knitr::kable(control_weakness, row.names = NA, 
             col.names = c("Domain", "Frequency Threat Overcomes Control"), 
             align = c("l", "r"))
```
...
of interest (e.g. ransomware) to your decission makers, high visibility 
scenarios can be analyzed and treated without loosing sight of the overall 
risk picture.
...
scenario_details <- scenario_summary %>% filter(scenario_id == 1)
```
...
SOMETHING INTERSTING ABOUT A KEY REDACTED SCENARIO

```{r scenarioB_details, echo=FALSE}
scenario_details <- scenario_summary %>% filter(scenario_id == 17)
```
...
Document the key risk manamgenet projects for the coming planning period 
...
the amount of loss exposure addresed by the control.
...
information and to reduce uncertainy, not to provide perfect information and 
compelte confidence. Attempting to do the latter results in an inability to 
...
multiple controls associated with the risk statemetn, the arithmatic mean across 
...
1. Number of times a threat actor acts against the organzation
...
gg <- gg + geom_hline(aes(yintercept = risk_tolerance["high"]), color = "red") +
  annotate("text", x = first_domain, y = risk_tolerance["high"], label = "High", vjust = -1, 
           hjust = 0)
gg <- gg + geom_hline(aes(yintercept = risk_tolerance["medium"]), color = "lightsteelblue") +
  annotate("text", x = first_domain, y = risk_tolerance["medium"], label = "Medium", 
           vjust = -1, hjust = 0)
gg <- gg + stat_boxplot(geom = 'errorbar', width = 0.5)
...
Repeating the above plot across all the scenarios with only the iutliers removed 
