...
```
```{r}
control_weakness %>% mutate(domain_id = factor(domain_id, 
                                               levels=rev(unique(domain_id)),
                                               ordered= TRUE)) %>% 
  ggplot(., aes(x = -loss_events, xend=(threat_events - loss_events), 
                y=domain_id,
                group=domain_id)) -> gg
gg <- gg + geom_dumbbell()             
gg <- gg + theme_bw()
...
gg <- gg + labs(x=NULL, y=NULL, title = "Loss Events vs Threat Events")
...
gg <- gg + labs(x = "Loss Events Per Year", 
                ...
                y = element_blank(),
                caption = "All Scenarios Included")
gg <- gg + theme_minimal()
