...
if ("pacman" %in% installed.packages()) {
  ...
  library(ggplot2)
  library(extrafont)
  library(ggalt)   # needs to be hrbrmstr/ggalt
  library(scales)
  library(hexbin)
  library(viridis)
  library(dplyr)
...
```{r enhance_summary_object_w_tolerances}
scenario_summary %<>% 
  mutate(annual_tolerance = ifelse(ale_median >= risk_tolerance["high"], 
                                   "High", 
                                   ifelse(ale_median >= risk_tolerance["medium"], 
                                          "Medium", "Low"))) %>% 
  mutate(annual_tolerance = factor(annual_tolerance, levels = c("High", "Medium", "Low"), ordered = TRUE)) 
```
...
#default theme
theme_evaluator <- function(base_family="Arial Narrow") {
...
threshold during a given year **excluding outlier scenarios**. The 80% line 
shows that a loss of no less than `r dollar(quantile(max_loss$loss, .2))` 
occurs every four out of five years. This likely loss threshold with the 
...
gg <- gg + labs(x = "Chance of Equal or Greater Loss", y = "Loss", 
                ...
                caption = "Generated by the Evaluator toolkit")
print(gg)
```
...
tc_exceedance <- results %>% group_by(domain_id) %>% 
  ...
  mutate(tc_exceedance = ifelse(is.na(tc_exceedance), NA, percent(tc_exceedance / 100 )),
         diff_exceedance = ifelse(is.na(diff_exceedance), NA, percent(diff_exceedance / 100))) %>% 
...
Threats most frequently overcome the control capabilities, resulting in losses, in 
the domains of `r control_weakness[1, "domain"]`, 
...
```
```{r}
dat <- control_weakness %>% mutate(domain_id = factor(domain_id, 
...
dat <- dat %>% gather(type, events, c(loss_events, contained_events)) %>% 
  mutate(events = ifelse(type=="loss_events", -1 * events, events))
gg <- ggplot(dat, aes(x = events, xend=0, 
                y=domain_id,
                group=domain_id)) -> gg
gg <- gg + geom_lollipop(horizontal = TRUE, color = "steelblue", point.size = 2)             
gg <- gg + geom_label(aes(x = 0, y=domain_id, label=domain_id), size=3, label.size=0)
gg <- gg + geom_text(data = filter(dat, type=="loss_events"), 
                     aes(label=tc_exceedance, x = events, y=domain_id, group=domain_id), hjust="right", 
                     vjust = "middle", size = 3,
                     nudge_x= -10000)
gg <- gg + geom_text(data = filter(dat, type=="contained_events"), 
                     aes(label=paste(diff_exceedance, "foo"), x = events, y=domain_id, group=domain_id), 
                     vjust = "middle", hjust="left", size = 3,
                     nudge_x = 10000)
gg <- gg + theme_evaluator()
gg <- gg + scale_x_continuous(labels=comma)
gg <- gg + theme(panel.border = element_blank())
gg <- gg + labs(x=NULL, y=NULL, 
                title = "Events Resulting in Losses vs Contained",
                caption = "Generated by the Evaluator toolkit")
...
which your decission makers have expressed interest (e.g. ransomware), these 
scenarios can be analyzed without losing sight of the overall risk picture.
...
a framework for looking at the entire security program. These domains are:

```{r domain_table}
select(domains, "Domain ID" = domain_id, "Domain" = domain) %>%
  arrange(Domain) %>% pander(caption="Domain Listing", justify="left")
```

## Capabilities

The security team and key subject matter experts met and formed a consensus 
opinion on the maturity level of the `r nrow(capabilities)` capabilities 
which make up over the mary$domain_id))` security program areas. The 
...
These capabiltiies and their ratings are:
...
arrange(scenarios, domain_id, scenario_id) %>% 
  ...
  select("Scenario ID" = scenario_id, "Scenario" = scenario, "Actor" = tcomm, "Actor Frequency" = tef, "Impact" = lm) %>% 
...
assigned to the threat scenario. Whether a loss occurs is a function of:
...
Scenarios are grouped based upon the frequency by which losses occur and the 
size of those losses. This gives four buckets for detailed analysis:
...
- Handled Scenarios: Scenarios which occur infrequently (if at all), and which 
have low impact upon the organization when they do occur.



```{r frequency_and_impact_plot}
gg <- ggplot(scenario_summary, aes(x=loss_events_median, y=sle_mean, size=ale_median, 
                color=annual_tolerance, text=scenario_id))
gg <- gg + geom_point()
#gg <- gg + geom_text(data=filter(gg$data, annual_tolerance != "Low"), aes(loss_events_median, sle_median, label=scenario_id))
...
gg <- gg + scale_color_brewer(palette = "Set1", type = "qual", drop= FALSE)
...
gg <- ggplot(dat, aes(x=loss_events, y=sle_median, size=ale ))
...
gg <- gg + labs(title = "Loss Frequency vs Magnitude",
           ...
           x = "Median Events per Year",
           y = "Median Event Magnitude")
gg <- gg + scale_x_discrete(labels = comma)
gg <- gg + scale_y_continuous(labels = dollar)
gg <- gg + scale_color_brewer(palette = "Set1", type = "qual", drop= FALSE)
gg <- gg + scale_size_continuous(labels = dollar_millions)
gg <- gg + guides(size = guide_legend(title="Median Annual Loss"))
gg <- gg + guides(color = guide_legend(title="Total Losses vs Risk Tolerance"))
gg <- gg + theme_evaluator(base_family="Myanmar Text")
gg
```
...
gg <- gg + labs(x = "Loss Events Per Year", 
                title = "Median Loss Events by Domain",
                y = element_blank(),
...
gg <- gg + labs(x = "Domain / Scenario ID", y = "Annual Expected Loss", 
                title = "Influence of Outliers on Domain Summaries",
...
gg <- gg + scale_y_log10(labels = dollar_millions)
gg <- gg + theme_evaluator() 
