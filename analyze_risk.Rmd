...
loss_exceedance <- results %>% 
  ...
  mutate(tolerance = ifelse(loss >= risk_tolerance["high"], "High", 
                            ifelse(loss >= risk_tolerance["medium"], "Low"))) %>% 
  count(tolerance)
```

Total potential losses for a given year are estimated to exceed the 
institution's high risk threshold `r percent(loss_exceedance[[loss_exceedance$tolerance == "High", "n"]] / max(results$simulation))` of the time.
...
dat <- map_df(loss_points,
              ~ data_frame(loss = .x, 
                           prob = sum(max_loss$loss > .x) /  nrow(max_loss),
                           outliers = FALSE))
...
mutate_each(domain_impact, funs(dollar), -domain_id, -domain) %>% 
  select("Domain" = domain, "Minimum Annual Loss" = min,
...
3. The difficulty the relevant controls present to the threat actor. For 
    scenarios which have multiple controls applied, the arithmatic mean of 
    eac of the applied simulated capabilities is used.
...
Risk is the total annualized expected loss across all the scenarios in a domain. 

## Assumptions

Each of the sets of parameters assigned to the qualitative labels is 
displayed below.
...
#gg <- gg + geom_text(data=filter(gg$data, annual_tolerance != "Low"), 
#                     aes(loss_events_median, sle_median, label=scenario_id))
gg <- gg + labs(title = "Loss Frequency vs Magnitude",
           subtitle = "All Scenarios",
           x = "Median Events per Year",
           y = "Median Event Magnitude")
