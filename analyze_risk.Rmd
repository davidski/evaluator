...
mutate_each(domain_impact, funs(dollar), -domain_id) %>% 
  knitr::kable(row.names = NA, 
               col.names = c("Domain", "Mean Annual Loss", 
                             "Maximum Annual Loss", 
...
create_table <- function(label, id) {
  ...
  filter(mappings, type == id) %>% select(label, l, ml, h, conf) %>% 
    ...
    knitr::kable(., row.names = NA, 
               col.names = c("Category", "Low", "Most Likely", "High", 
                             "Confidence"), 
               align = c("l", "r", "r", "r", "r"),
...
scenarios %>% rowwise %>% 
  ...
  pander::pander(., justify = c("left", "left", "left"), caption="Fragile Scenarios")
```
...
left_join(scenarios, scenario_summary, by = c("scenario_id" = "scenario_id",
  ...
  select("ID" = domain_id, "Scenario" = scenario, 
