...
The top scenarios, measured by value at risk, are:
...
gg <- gg + geom_vline(xintercept = 0.8, color = "red", size = 1.7) + 
...
break_locations = c(max(dat$loss_events) / 2 * -1, max(dat$contained_events) / 2 * 1.5)
...
a framework for systematically reviewing risk. These domains are:
...
gg <- gg + geom_tile()
gg <- gg + geom_text(aes(x = domain, y = 1, label = full_label), color = "white")
gg <- gg + coord_equal()
gg <- gg + scale_fill_viridis(guide = guide_legend(title=NULL), labels=dollar_millions)
gg <- gg + labs(x = NULL, y=NULL,
...
reducing loss impact, or transferring risk. Describe each project in terms of 
its expected cost versus the amount of loss exposure addressed by the control.
