...
institution's major risk threshold (`r dollar(risk_tolerance["high"])`) 
...
The following table shows the maximum, 95th percentile Value at Risk (VaR), mean, and minimum single year losses.

```{r overall_exposure, echo=FALSE}
overall <- results %>% group_by(simulation) %>% 
                     summarize(ale = sum(ale)) %>% select(ale) %>%
                     ungroup
data_frame("95% VaR" = dollar(quantile(overall$ale, c(0.95))),
...
## Top Five Risk Scenarios

The top scenarios, measured by a standard financial 95% Value at Risk (VaR) 
calculation, are:
...
top_n(scenario_summary, 5, ale_var) %>% 
  ...
  select(Domain = domain_id, Scenario = scenario, 
...
```
The full list of evaluated risk scearios is located in [Appendix A](#scenario_list).
...
where this occurs are clearly noted. Outlier scenarios are:
...
filter(scenario_summary, outlier == TRUE) %>% 
  ...
  arrange(desc(ale_var)) %>% mutate_at(c("ale_var", "ale_median", "ale_max"), dollar) %>% 
  rename("Domain ID" = domain_id, ID = scenario_id, "Description" = scenario, "VaR" = ale_var, 
...
outliers included is `r dollar(quantile(filter(max_loss, outliers==TRUE)$max_loss, .2))`.
...
max_loss %>% 
  ...
  ggplot(., aes(prob, max_loss, color = outliers)) + 
  geom_path() -> gg
gg <- gg + geom_vline(xintercept = 0.8, color = "red", size = 1.1) + 
  annotate("text", x = 0.8, y = max(max_loss$max_loss), label = percent(.8),
                               labels = c("Excluded", "Included"))
gg <- gg + scale_color_viridis(discrete = TRUE, 
                               labels = c("Excluded", "Included"), 
                               guide = FALSE)
#gg <- gg + geom_text(dat = (filter(gg$data, outliers==FALSE)), aes(x = 0, y = max(max_loss), label = "Foo"))
...
mutate_each(domain_impact, funs(dollar), -domain_id, -domain) %>% 
  select("Domain" = domain, "Value at Risk" = var, "Minimum" = min,
...
gg <- gg + labs(x = NULL, y = NULL, 
                title = "Event Outcomes by Domain",
                subtitle = "Losses/Contained (Control Gap/Surplus)",
                caption = "Source: Evaluator toolkit")
gg <- gg + theme_evaluator()
...
a framework for systematically reviewng risk. These domains are:
...
This section helps the analyst review where the organization's security risk 
is concentrated.
...
Histogram of annual losses across all scenarios and simulations. Zero 
