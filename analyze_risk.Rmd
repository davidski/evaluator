...
and tables with such exclusion are noted.
...
mutate_each(domain_impact, funs(dollar), -domain_id, -domain) %>% 
  ...
  pander::pander(justify = c(rep("left", 1), rep("right", 3)),
                 split.cells = 25,
...
gg <- gg + geom_boxplot(fill = viridis(1), alpha = 1/5, 
                        outlier.color = alpha(viridis(1), 1/2), 
                        outlier.shape = 16)
gg <- gg + theme_minimal()
gg <- gg + theme(panel.grid.major = element_blank())
gg <- gg + theme(panel.grid.minor = element_blank())
...
gg <- gg + stat_boxplot(geom = 'errorbar', width = 0.5)
gg <- gg + geom_boxplot(fill = viridis(1), alpha = 1/5, 
                        outlier.color = alpha("black", 1/40), outlier.shape = 16)
gg <- gg + theme_minimal()
gg <- gg + theme(panel.grid.major = element_blank())
gg <- gg + theme(panel.grid.minor = element_blank())
...
gg <- gg + labs(x = "Risk Exposure", 
                y = element_blank(), 
                title = "Risk Exposure",
                subtitle = "Outlier Scenarios Included")
gg <- gg + theme_minimal()
gg <- gg + theme(panel.grid.major = element_blank())
gg <- gg + theme(panel.grid.minor = element_blank())
