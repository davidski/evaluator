...
```{r top_5_risks}
top_n(scenario_summary, 5, ale_median) %>% 
  left_join(scenarios, by = c("scenario_id" = "scenario_id", 
                              "domain_id" = "domain_id")) %>% 
  mutate(ale_max = dollar(ale_max), ale_median = dollar(ale_median)) %>% 
  select(Domain = domain_id, Scenario = scenario, 
         "Median Annual Loss" = ale_median, "Maximum Annual Loss" = ale_max) %>% 
