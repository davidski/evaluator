...
results <- by_row(scenarios, function(x) {
  ...
  safe_calculate <- safely(calculate_ale)
...
y <- results$`.out` %>% transpose()
is_ok <- y$error %>% map_lgl(is_null)

if (sum(is_ok) != nrow(scenarios)) {
  stop("Error encountered with with scenario IDs: ", paste0(scenarios[!is_ok,]$scenario_id, collapse = ", "))
}

results <- y$result %>% bind_rows
```
