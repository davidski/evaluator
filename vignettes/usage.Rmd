...
`import_spreadsheet("PATH", domains)`. Evaluator The notebook will extract the data. 
...
domains <- system.file("extdata", "domains.csv", package="evaluator") %>% 
  read_csv()
...
### Validate the Data
`validate_scenarios()`. If there are data validation errors, the process will 
abort and an error message will be displayed. Correct the spreadsheet, reimport, 
and repeat validation process until there are no errors. 
 
```{r validate, eval=FALSE}
mappings <- system.file("extdata", "qualitative_mappings.csv", package="evaluator") %>% readr::read_csv()
...
computationally intense. The sample data set takes approximately five minutes on 
my primary development machines (last generation Windows-based platforms).

```{r simulate, eval = FALSE}
results <- run_simulations(quantitative_scenarios, simulation_count = 1000L)
```
