...
new_evaluator_scen <- function(tef_params = list(),
  ...
  scenario <- list(tef_params = tef_params,
                   ...
                   lm_params = lm_params,
                   model = model)
  class(scenario) <- c("evaluator_scen") #class(scenario))
  scenario
}

#' @export
#' @rdname new_evaluator_scen
evaluator_scen <- function(tef_params = list(), tc_params = list(),
                          diff_params = list(), lm_params = list(),
                          model = "openfair_tef_tc_diff_lm") {
  new_evaluator_scen(tef_params, tc_params, diff_params, lm_params, model)
}

#' @export
#' @importFrom cli cat_line
print.evaluator_scen <- function(x, ...) {
