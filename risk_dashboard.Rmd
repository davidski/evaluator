...
### Top 10 Scenarios with Simulated Losses Ranges {.no-padding}
...
minimal_dat <- filter(dat, ale!=0) %>% group_by(full_label) %>% 
  mutate(ale_median = median(ale)) %>% ungroup %>% 
  mutate(full_label = fct_reorder(full_label, ale_median))

# plot!
gg <- ggplot(minimal_dat, aes(x = full_label, y = ale + 1))
