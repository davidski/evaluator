...
Linux). If the `pacman` package is installed (`install.packages("pacman")`), 
Evaluator will auto-install any missing dependencies. This README assumes the 
use of [RStudio IDE](https://www.rstudio.com/), but it is not strictly 
required (advanced users may manually `knit` files if they so choose).

Obtain the Evaluator toolkit by either cloning this repository 
(`git clone https://github.com/davidski/evaluator`) or downloading and 
...
Medium/Low to High/Medium/Low/VeryLow) is unsupported.
