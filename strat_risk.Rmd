...
```{r support_setup}
pacman::p_load(tidyverse, pbapply, readr)
...
Set the number of simualations to run for each scenario in the `simulaton_count` 
variable.
...
strength for the entire domain. This sampled strength is added to each 
threat scenarios for the domain.
-->

```{r estimate_domain_difficulty, eval=FALSE}
...
derive_controls <- function(x, id="None") {
  ...
  results <- capabilities[capabilities$id %in% as.numeric(control_list), "diff"] %>% 
    rename(label=diff) %>% 
...
#control_params <- mapply(derive_controls, scenarios$controls, 
#                         scenarios$scenario_id, SIMPLIFY = FALSE)
...
is returned with a bit of extra metadata to note the date when these 
...
scenario_summary <- results %>% group_by(domain_id, scenario_id) %>% 
  summarise(
    loss_events = sum(loss_events, na.rm = TRUE),
...
Also aggregate all the scenarios in a domain to a single maximum expected 
loss (ALE).
...
Save out the detailed results (`results`), per-scenario summaries 
(`scenario_summary`), domain summaries (`domains_summary`), and the 
...
file.info(c("data/results.Rdata", "data/scenario_summary.Rdata", 
            "data/domain_summary.Rdata", "data/scenarios_full.Rds"))
```